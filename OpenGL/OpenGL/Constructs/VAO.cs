...
namespace klukule.OpenGL
{
    ...
    public class GenericVAO : IDisposable
    {
        ...
        /// <summary>
        /// The ID of this Vertex Array Object for use in calls to klukule.OpenGL.
        /// </summary>
        ...
        public void Dispose()
        {
            ...
            // children must be disposed of separately since klukule.OpenGL 2.1 will not have a vertex array
            if (DisposeChildren)
            {
                for (int i = 0; i < vbos.Length; i++)
                {
    ...
    public class VAO : IDisposable
    {
        ...
        public VAO(ShaderProgram program, VBO<Vector3> vertex, VBO<Vector3> normal, VBO<Vector3> tangent, VBO<Vector2> uv, VBO<int> element)
        {
            ...
            if (Gl.Version() >= 3)
            {
                ...
                Draw = DrawOGL3;
                DrawArrays = DrawOGL3Array;
            }
            else
            {
                Draw = DrawOGL2;
                DrawArrays = DrawOGL2Array;
            }
        }
        ...
        /// klukule.OpenGL.dll assumes the common naming conventions below:
        ...
        public delegate void DrawFunc();
        public delegate void DrawFuncArray(int start,int count);

        public DrawFunc Draw;
        public DrawFuncArray DrawArrays;

        private void DrawOGL3Array(int start, int count)
        {
            if (vaoID == 0) return;
            Gl.BindVertexArray(vaoID);
        ...
        private void DrawOGL2Array(int start, int count)
        {
            BindAttributes(this.Program);
