...
namespace klukule.OpenGL
{
    public class Texture : IDisposable
    {
        ...
        public Texture(uint TextureID, Size Size,string Filename, TextureTarget TextureTaget = TextureTarget.Texture2D)
        {
        ...
        private void LoadBitmap(Bitmap BitmapImage, bool FlipY = true)
        {
             ...
             * that the garbage collector doesn't touch it, and it is read via klukule.OpenGL glTexImage2D. */
            ...
            Gl.BindTexture(TextureTarget, TextureID);     // bind the texture to memory in klukule.OpenGL
            ...
            Gl.TexParameteri(TextureTarget, TextureParameterName.TextureMinFilter, TextureParameter.Linear);//(int)TextureParam.Linear);   // linear filter
        ...
        /// <summary>
        /// Loads a compressed DDS file into an klukule.OpenGL texture.
        /// </summary>
        ...
        private void LoadDDS(string ResourceFile)
        {
            using (BinaryReader stream = new BinaryReader(new FileStream(ResourceFile, FileMode.Open)))
            {
                ...
                // doesn't shuffle the data around while klukule.OpenGL is decompressing it
